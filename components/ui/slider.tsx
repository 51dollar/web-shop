    ...
    >
      <SliderPrimitive.Track
        ...
        className="bg-muted relative grow overflow-hidden rounded-full
             ...
             data-vertical:w-1 data-vertical:h-full"
      >
      ...
      {Array.from({ length: _values.length }, (_, index) => (
        <SliderPrimitive.Thumb
          ...
          key={index}
          className="border-primary ring-ring/50 size-3 rounded-4xl border bg-white shadow-sm transition-colors
              hover:ring-4 focus-visible:ring-4 focus-visible:outline-hidden 
              block shrink-0 select-none disabled:pointer-events-none disabled:opacity-50"
        />
      ))}
    </SliderPrimitive.Root>
  )
